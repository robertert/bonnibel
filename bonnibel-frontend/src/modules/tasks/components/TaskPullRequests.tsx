...
export default function TaskPullRequests({ projectId, taskId, onChanged, userRole }: { projectId: number; taskId: number; onChanged?: () => void; userRole?: ProjectRole | null }) {
  ...
  const load = () => {
    setLoading(true)
    pullRequestService.getTaskPullRequest(projectId, taskId)
      .then((pr) => setPrs(pr ? [pr] : []))
      .catch(() => setPrs([]))
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setBusy(true)
    try {
      await pullRequestService.createPullRequest(projectId, taskId)
  ...
  const handleDecision = async (prId: number, action: 'approve' | 'decline') => {
    let reason = ''
    if (action === 'decline') {
      const input = window.prompt('Podaj powód odrzucenia:')?.trim()
      if (!input) return
      reason = input
    }
    setBusy(true)
    try {
      if (action === 'approve') await pullRequestService.acceptReview(projectId, prId)
      else await pullRequestService.rejectReview(projectId, prId, reason)
      load(); onChanged?.()
