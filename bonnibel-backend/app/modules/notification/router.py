...
@router.post("/tasks/{task_id}/subscribe", response_model=TaskSubscriptionRead, status_code=status.HTTP_201_CREATED)
...
@router.get("/tasks/{task_id}/messages", response_model=list[ChatMessageRead])
async def get_task_messages(
    task_id: int,
    current_user_id: CurrentUserId,
    db: Annotated[Session, Depends(get_db)],
    limit: Annotated[int, Query(ge=1, le=200)] = 100,
    before_id: Annotated[int | None, Query(alias="beforeId")] = None,
) -> list[ChatMessageRead]:
    return notification_service.list_messages(db, task_id, limit=limit, before_id=before_id)


@router.post("/tasks/{task_id}/messages", response_model=ChatMessageRead, status_code=status.HTTP_201_CREATED)
async def send_task_message(
    ...
    return await notification_service.send_message(db, task_id, current_user_id, payload.text)
...
@router.post("/notifications/demo/task-snapshots", status_code=status.HTTP_204_NO_CONTENT)
